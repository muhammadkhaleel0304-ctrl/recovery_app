...
<head>
    ...
    <style>
        ...
        .card:hover {
            transform: translateY(-10px) rotateX(10px) rotateY(5px);
        ...
        h1 {
            font-size: 2rem;
            background: linear-gradient(45deg, #ff69b4, #ffb6c1, #8a2be2);
        ...
        .card-container.open .secret-message {
            opacity: 1;
...
<body>
    ...
    <div class="card-container" onclick="toggleCard()">
        ...
        <div class="secret-message">
            ...
            <p>Aapki zindagi ka har lamha khushiyon se bhara ho. Aap jo bhi chahein, wo sab aapko mile. Hamesha haste muskurate rahein! 💖</p>
        </div>
    </div>

    <script>
        // Toggle the card open/close animation
        function toggleCard() {
            document.querySelector('.card-container').classList.toggle('open');
        }

        // Dynamically create floating balloons
        function createBalloons() {
            const colors = ['#ff69b4', '#ffb6c1', '#8a2be2', '#4b0082', '#00f2fe', '#4facfe', '#ff0844'];
            for (let i = 0; i < 20; i++) {
                const balloon = document.createElement('div');
                balloon.classList.add('balloon');
                
                // Randomize background color, horizontal position, delay, and size
                ...
                balloon.style.boxShadow = `inset -5px -5px 10px rgba(0,0,0,0.2), 0 5px 15px ${randomColor}44`;
